...
상품 등록 시 판매 시작 시각(`saleStartAt`)을 미래로 지정해두면, 해당 시각이 도래해야 실제 판매가 시작되어야 한다. 이를 처리하기 위해 product-service에 스케줄러를 구현했다.

- 1분 주기로 실행되며, `INACTIVE` 상태인 상품 중 `saleStartAt`이 현재 시각 이전인 상품을 만료 대상이 아닌 활성화 대상으로 조회
- 대상 상품을 `ACTIVE`로 상태 전이하고 `product.activated` 이벤트 발행
- 한 회 실행에서 처리할 최대 건수는 `product.activation.batch-size`(기본값 100)로 조정 가능
