...
- Event Type: `ProductActivatedEvent`
...
상품이 판매 가능 상태로 전이되었음을 알리는 이벤트다.

현재 프로젝트 기준 등록된 consumer는 없지만, 카탈로그 동기화나 검색 인덱싱 같은 후속 확장 지점으로 사용할 수 있다.
...
|---|---|---|---|
| meta.eventType | string | yes | `ProductActivatedEvent` |
...
{
  "meta": {
    "eventId": "evt-product-activated-001",
    "eventType": "ProductActivatedEvent",
